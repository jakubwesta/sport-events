import { useEffect, useRef } from 'react'
import { useGoogleOAuth } from '@react-oauth/google'

import { Button } from '@/components/ui/button'
import { googleClientId } from '@/lib/env'
import { cn } from '@/lib/utils'

type GoogleSignInButtonProps = {
  disabled?: boolean
  mode?: 'signin' | 'signup'
  onSuccess: (idToken: string) => void | Promise<void>
  onError?: () => void
}

type CredentialResponse = {
  credential?: string
}

declare global {
  interface Window {
    google?: {
      accounts: {
        id: {
          initialize: (config: {
            client_id: string
            callback: (response: CredentialResponse) => void
          }) => void
          renderButton: (
            parent: HTMLElement,
            options: {
              type?: string
              theme?: string
              size?: string
              text?: string
              width?: number
              shape?: string
            },
          ) => void
        }
      }
    }
  }
}

function GoogleMark() {
  return (
    <svg className="size-4 shrink-0" viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}

export function GoogleSignInButton({
  disabled = false,
  mode = 'signin',
  onSuccess,
  onError,
}: GoogleSignInButtonProps) {
  const { clientId, scriptLoadedSuccessfully } = useGoogleOAuth()
  const overlayRef = useRef<HTMLDivElement>(null)
  const onSuccessRef = useRef(onSuccess)
  const onErrorRef = useRef(onError)

  onSuccessRef.current = onSuccess
  onErrorRef.current = onError

  useEffect(() => {
    const overlay = overlayRef.current
    if (!scriptLoadedSuccessfully || !clientId || !overlay) return

    window.google?.accounts.id.initialize({
      client_id: clientId,
      callback: (response) => {
        if (response.credential) {
          void onSuccessRef.current(response.credential)
        } else {
          onErrorRef.current?.()
        }
      },
    })

    overlay.innerHTML = ''

    const render = () => {
      if (!overlayRef.current) return
      window.google?.accounts.id.renderButton(overlayRef.current, {
        type: 'standard',
        theme: 'outline',
        size: 'large',
        shape: 'rectangular',
        text: mode === 'signup' ? 'signup_with' : 'signin_with',
        width: Math.max(overlayRef.current.offsetWidth, 280),
      })
    }

    render()

    const observer = new ResizeObserver(render)
    observer.observe(overlay)

    return () => observer.disconnect()
  }, [scriptLoadedSuccessfully, clientId, mode])

  if (!googleClientId) return null

  const label = mode === 'signup' ? 'Sign up with Google' : 'Sign in with Google'

  return (
    <div
      className={cn(
        'relative h-10 w-full',
        (disabled || !scriptLoadedSuccessfully) && 'pointer-events-none opacity-50',
      )}
    >
      <Button
        type="button"
        variant="default"
        size="lg"
        className="pointer-events-none absolute inset-0 z-0 w-full gap-2"
        tabIndex={-1}
        aria-hidden
      >
        <GoogleMark />
        {label}
      </Button>
      <div
        ref={overlayRef}
        className="absolute inset-0 z-10 overflow-hidden opacity-[0.01]"
        aria-label={label}
      />
    </div>
  )
}

export function isGoogleSignInEnabled() {
  return Boolean(googleClientId)
}
